    ...
    <v-layout row wrap :class="budgetsVisible ? 'l-budgets-header' : 'l-clients-header'">
      <v-flex xs12 md5>
        <v-text-field
          ...
          label="Search"
          append-icon="search"
          :color="budgetsVisible ? 'light-blue light-l' : 'green light-l'">
        </v-text-field>
      </v-flex>
      ...
      <v-flex xs12 offset-md1 md1>
        <v-btn block color="red light-l white--text"
          @click.native="submitSignout()">Sign out</v-btn>
      </v-flex>
    </v-layout>
  </header>
</template>
<script>
  ...
  export default {
    ...
    methods: {
      submitSignout () {
        console.log('sign out debugging...')
